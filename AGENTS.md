...
Application skills are managed in `admin/skills.py` and `admin/skill_routes.py`. Import a single directory containing YAML-frontmatter `SKILL.md` and copy all resources into the application's OpenCode config directory under `skills/<name>`. Disabled copies live in the adjacent `skills-disabled/<name>` directory, outside native skill scan roots; deletion only removes the application copy. The UI is in `web/static/skills.js`, with the skills navigation immediately after workspace. `/skills` selects an enabled application skill and fills `/skill-name`; invocation uses V1 `/session/{id}/command`, while automatic selection uses OpenCode's native `skill` tool. Do not replace native invocation with pasted skill text, which loses resource base-directory behavior. Skill changes reject active workspace tasks and refresh idle instance caches through `/instance/dispose` (restart idle processes if unsupported). Existing terminal sessions must be restarted to refresh skills. Cover copying, enablement persistence, traversal/symlink rejection, busy-task protection, cache refresh, and command delegation in `tests/test_skills.py`.
